...
import DesktopOrderConfirmation from "../components/order/DesktopOrderConfirmation";
...
const PaymentStatusPage = () => {
    ...
    const verifyPayment = async () => {
        ...
        try {
            ...
            if (isNetworkError) {
                setStatus("timeout");
                setError("Cannot reach the backend. If Razorpay opened this on another device/app, localhost URLs will not work.");
                if (pollInterval.current) clearInterval(pollInterval.current);
                return;
            }
    ...
    if (status === "success") {
        const targetId = orderDetails?.checkoutGroupId || orderDetails?.publicOrderId || orderDetails?.order || merchantOrderId;
        return (
            <div>
                <div className="md:hidden min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans">
                    <motion.div 
                ...
                <div className="hidden md:block">
                    <DesktopOrderConfirmation
                        orderId={targetId}
                        totalAmount={orderDetails?.amount || 1226}
                        itemCount={orderDetails?.items?.length || 4}
                    />
                </div>
            </div>
        );
